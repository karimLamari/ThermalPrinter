...
const { listBluetoothDevices, testNetworkPrinter } = require('./printer');
...
app.post('/api/config', async (req, res) => {
  ...
  try {
    // Tester la connexion WiFi AVANT de sauvegarder
    console.log('Test de connexion WiFi...');
    const connected = await connectToWifi(wifiSSID, wifiPassword);

    if (!connected) {
      return res.status(400).json({
        error: 'Impossible de se connecter au WiFi',
        details: 'Vérifiez le nom du réseau et le mot de passe'
      });
    }
    ...
    if (!internet) {
      return res.status(400).json({
        error: 'WiFi connecté mais pas d\'accès internet',
        details: 'Le réseau WiFi n\'a pas accès à internet'
      });
    }
    ...
    res.json({
      success: true,
      message: 'Configuration enregistrée. Redémarrage dans 3 secondes...'
    });
